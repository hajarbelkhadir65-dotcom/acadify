...
export default function SupervisorParametres({ user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation simple pour le changement de mot de passe
    ...
    try {
      ...
      const response = await axios.put('/api/supervisor/profile', {
      ...
      if (response.data.success) {
        ...
        // Synchro locale : Met à jour l'objet utilisateur dans le localStorage
        localStorage.setItem('user', JSON.stringify(response.data.user));
        
        // Réinitialise uniquement les champs de mots de passe
        setFormData(prev => ({
          ...prev,
          currentPassword: '',
          newPassword: '',
        ...
        // Optionnel : Tu peux forcer un rechargement de la page si tu veux rafraîchir le header global instantanément
        window.location.reload();
      }
    } catch (error) {
      console.error("Erreur lors de la modification du profil encadrant :", error);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-xs text-gray-400 font-medium">Gérez vos informations de profil professionnelles et vos accès de sécurité.</p>
      </div>
